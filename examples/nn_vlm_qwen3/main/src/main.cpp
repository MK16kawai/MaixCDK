...
void on_reply(nn::Qwen3VL &obj, const nn::Qwen3VLResp &resp)
{
...
int _main(int argc, char* argv[])
{
    ...
    nn::Qwen3VL qwen3vl(model_path);
    const char *system_prompt = "You are Qwen3VL, a multimodal model and a helpful vision-to-text assistant.";
    if (argc > 2)
    {
        system_prompt = argv[2];
    }
    log::info("System prompt: %s", system_prompt);
    qwen3vl.set_system_prompt(system_prompt);
    qwen3vl.set_reply_callback(on_reply);
    ...
    while(!app::need_exit()) {
        ...
        log::info("Qwen3VL is loading...");
        time::sleep_ms(1000);
    }
    log::info("Qwen3VL loaded");
    ...
    while(!app::need_exit())
    {
        ...
        if(!input.empty())
        {
            ...
            err::Err e = qwen3vl.set_image(*img, maix::image::Fit::FIT_CONTAIN);
            delete img;
        ...
        // send message to Qwen3VL
        nn::Qwen3VLResp resp = qwen3vl.send(prompt);
